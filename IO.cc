...
struct PLAYER_NAME : public Player {
    ...
    /**
     * Represents a kame direction with its scores.
     */
    struct KameDir
    {
        /**
         * Direction of the kame.
         */
        Dir dir;

        /**
         * Score of gokus inside the dangerous areas.
         */
        double score_in;

        /**
         * Score of gokus outside the dangerous areas.
         */
        double score_out;

        /**
         * Creates an empty kame direction.
         * @return A new empty kame direction.
         */
        inline KameDir()
        :   dir(None),
            score_in(0.0),
        ...
        /**
         * Comparison operator to compare two kame directions.
         * @return True if a is a better kame direction than b,
         * false otherwise.
         */
        inline friend bool operator> (const KameDir& a, const KameDir& b)
        {
            double max;

            if(a.score_in > a.score_out)
                max = a.score_in;
            else
                max = a.score_out;
            
            return (max > b.score_in and max > b.score_out);
        }

        /**
         * Returns whether the kame direction has no scores (no gokus aligned).
         * @return True if the kame direction is empty, false otherwise.
         */
        bool empty()
        {
            return (score_in <= 0.0 and score_out <= 0.0);
        }
    };
     ...
     *             aligned.
     */
    Dir get_kame_direction()
    {
        KameDir kame;

        double potential_balls = double(nb_rounds() - round()) / ROUNDS_PER_BALL;
        
        kame.score_out = 0 - potential_balls;
        kame.score_in = double(gme.balls) - potential_balls + strength_prop(gme);
        ...
        for(int i = Top; i <= Right; ++i)
            kame_dir_update(static_cast<Dir>(i), gokus, kame);
     ...
     * @param kame        Current best kame direction
     */
    void kame_dir_update(const Dir &d, const vector<bool> &radar_gokus, KameDir &kame)
    {
        ...
        while(cell(u).type != Rock)
        {
            ...
            if(gid >= 0)
                update_score(current, gid, radar_gokus[gid]);
